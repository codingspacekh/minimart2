...
public class OrderScreen extends JFrame {
    ...
    private void addToOrder() {
        ...
        if (code.isEmpty() || amtStr.isEmpty()) {
            setStatus("Please enter the product code and amount", Color.RED);
            return;
        }

        Product product = productDAO.getProductByCode(code);
        if (product == null) {
            setStatus("Product is not existed", Color.RED);
            return;
        }
        setStatus("Product existed", Color.GRAY);
        cartItems.add(new OrderItem(code, product.getName(), Integer.parseInt(amtStr), product.getPrice(), product.getThumbnail()));
        refreshTable();
