...
public class OrderScreen extends JFrame {
    ...
    private void addToOrder() {
        String code   = getFieldValue(codeField, "Enter product code");
        String amtStr = getFieldValue(amountField, "Enter the amount");

        if (code.isEmpty() || amtStr.isEmpty()) {
            setStatus("Please enter product code and amount.", Color.RED);
            return;
        }
        ...
        try {
            ...
            setStatus("Amount must be a whole number.", Color.RED);
            return;
        }
        if (amount <= 0) {
        ...
        if (product == null) {
            setStatus("Product not found.", Color.RED);
            return;
        }
        setStatus("product existed", Color.GRAY);

        int alreadyInCart = 0;
        OrderItem existing = findCartItem(code);
        if (existing != null) alreadyInCart = existing.getAmount();

        if (amount + alreadyInCart > product.getAmount()) {
            setStatus("Not enough stock available.", Color.RED);
            return;
        }
        ...
        if (existing != null) {
            ...
            cartItems.add(new OrderItem(product.getCode(), product.getName(), amount, product.getPrice(), product.getThumbnail()));
        }
    ...
    private OrderItem findCartItem(String code) {
        for (OrderItem item : cartItems) {
            if (item.getProductCode().equals(code)) return item;
        }
        return null;
    }
    ...
    private void updateSelectedItem() {
        if (selectedRow < 0 || selectedRow >= cartItems.size()) {
            setStatus("Select a row to update.", Color.RED);
            return;
        }

        String amtStr = getFieldValue(amountField, "Enter the amount");
        if (amtStr.isEmpty()) {
            setStatus("Enter the new amount.", Color.RED);
            return;
        }
        ...
        OrderItem item = cartItems.get(selectedRow);
        Product product = productDAO.getProductByCode(item.getProductCode());
        if (product == null) {
            setStatus("Product not found.", Color.RED);
            return;
        }
    ...
    private void deleteSelectedItem() {
        if (selectedRow < 0 || selectedRow >= cartItems.size()) {
            setStatus("Select a row to delete.", Color.RED);
            return;
        }
    ...
    private void onRowSelected() {
        ...
        setFieldValue(codeField, "Enter product code", item.getProductCode());
        setFieldValue(amountField, "Enter the amount", String.valueOf(item.getAmount()));
        setStatus("product existed", Color.GRAY);
    }
    ...
    private void placeOrder() {
        if (cartItems.isEmpty()) {
            setStatus("Add at least one product to the order.", Color.RED);
            return;
        }

        double total = computeTotal();
        int confirm = JOptionPane.showConfirmDialog(
            this, "Place this order for $" + PRICE_FMT.format(total) + "?", "Confirm Order",
            JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE
        );
        if (confirm != JOptionPane.YES_OPTION) return;

        int orderId = orderDAO.createOrder(currentUser.getUserId(), total, cartItems);
        if (orderId == -1) {
            showError("Failed to place order. Please check stock and try again.");
            return;
        }

        showInfo("Order placed successfully! Order #" + orderId);
        cartItems.clear();
        refreshTable();
        clearSelectionAndFields();
    }
